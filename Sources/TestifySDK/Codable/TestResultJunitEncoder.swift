...
public struct TestResultJunitEncoder: TestResultEncoder {
    ...
    public func encode(_ input: TestSuite) throws -> String {
        ...
        for suite in suites {
            ...
            restOfResult +=
                "<testsuite  id=\"\(name)\" name=\"\(name)\" tests=\"\(tests)\" skipped=\"0\" failures=\"\(failureCount)\" errors=\"0\" timestamp=\"\(start)\" hostname=\"JunitEncoder\" time=\"\(time)\">\n"
            ...
            for testCase in suite.cases {
                ...
                if success {
                    ...
                    if testCase.failureInfo != nil {
                        restOfResult += ">\n"
                        restOfResult +=
                            "<failure message=\"\(failLine ?? -1)\" type=\"type\">\n"
        ...
        startResult +=
            "<testsuites id=\"\(formatter.string(from: input.startDate))\" name=\"\(input.name)\" tests=\"\(allTests)\" failures=\"\(allFails)\" time=\"\(allTimes)\">\n"
